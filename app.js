...
function initMap() {
  ...
  const map = new google.maps.Map(document.getElementById('map'), {
    zoom: 13,
    center: {lat: -23.4418663, lng: -47.5389033}, 
    styles: mapStyle,
  });
  ...
  const apiKey = 'AIzaSyD-HCL5MJxX7S-XGbY9vI_E5z2ddNnvrzY';
  ...
  map.data.addListener('click', (event) => {
    ...
    const endereco = event.feature.getProperty('endereco');
    const position = event.feature.getGeometry().get();
    const content = sanitizeHTML`
      <img style="float:left; width:100px; margin-top:0px" src="img/logo_${category}.png">
      <div style="margin-left:120px; margin-bottom:0px;">
        <h2>${name}</h2><p>${description}</p>
        <p><b>Horário de Funcionamento:</b> ${hours}<br/><b>Endereço:</b> ${endereco}<br/><b>Telefone:</b> ${phone}</p>
      </div>
      `;
  ...
  title.textContent = 'Descubra a loja mais próxima';
  titleBar.appendChild(title);
  container.setAttribute('id', 'pac-container');
  input.setAttribute('id', 'pac-input');
  input.setAttribute('type', 'text');
  input.setAttribute('placeholder', 'Digite seu endereço');
  ...
  autocomplete.addListener('place_changed', async () => {
    ...
    if (!place.geometry) {
      ...
      window.alert('Insira o endereço mais próximo da sua localização!');
      return;
    }
